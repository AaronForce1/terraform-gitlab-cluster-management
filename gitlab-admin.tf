...
locals {
  ...
  curl_cmd = join("", ["curl -s --header \"Private-Token: ${var.gitlab_configuration.token}\" ",
    local.group_cluster_api_url,
  " -H 'Content-Type:application/json' --request PUT --data '{\"management_project_id\":\"'${var.gitlab_configuration.cluster_management_project_id}'\"}'"])
}